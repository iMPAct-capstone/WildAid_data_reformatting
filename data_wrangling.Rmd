...
split <- fuzzyjoin::stringdist_join( 
  ...
  max_dist = 1,
  mode = "left", #left join
  method = "jw", #method of matching how closely related the two strings are
...
#repeating the process for sub_categories that are combined into one and need to be averaged in later years
...
file_cells <- xlsx_cells(file.path(data_path, "raw", file_list[4]), sheet = 1) #read the cells of each file
format_cells <- xlsx_formats(file.path(data_path, "raw", file_list[4])) #read the format of each file

file_name <- as.character(file_list[4])
```

#Reformat all the files This section creates a for loop which will read all the files in the file list above as both xlsx cells and xlsx formats. It runs each file in the list through the previously created function and adds the reformatted data to a list. Since impact indicator data is stored in a second sheet in some of the files, a second loop is run pulling and reformatting the data from the second when this is the case. This data is added to a second list. Once all files have been reformatted in the file list, reformatted files are added to one data frame and saved as MPS_tracker_data.
...
#Update the Site Names This section reads a lookup table with the shortened site names used in the files and the full name of the site. It converts the site name to the full name and reformats the country name to have spaces when needed.
...
let's do a quick test first to see what we can do:

```{r}
# test it out --------------
# just running this line creates a spreadsheet with a unique name that's really funny! "unpleasant-buffalo"
MPS_sheet <- write_sheet(data = MPS_tracker_data, # give it a data.frame object 
                         sheet = "test-mps")      # this argument specifies the sheet name, not file name
##### NA columns just get stored as 'blank'
#####--- our MPS_tracker_data data.frame file started with 316 rows and the google sheet has 317 rows ... this is because the data.frame object does not include the column names as a row, but the google sheet does 

# control file name - initialize sheet --------------------  
# ok I want to control what that file is called... this means i have to use the 'gs4_create()' function
# call this function first to initialize a google sheet and then use write_sheet to add data to it
test_gs4_sheet <- gs4_create("file_name", 
                             sheets = "sheet_name")

# try moving the spreadsheet ----------------------- 
# I am moving this file called "file name" into arbitary file in our shared google drive: "iMPAct_drive/data/write_to_here_gs4"
# ok now it's moved so let's add our data to it 

# write our data to the sheet in correct position within drive ------------------------ 
write_sheet(data = MPS_tracker_data,
            ss = test_gs4_sheet,
            sheet = "sheet_name")
# alright! I checked that file in our google drive and our data is there! yay :)

# let's move this test df we made to the trash --------------------
googledrive::drive_trash(test_gs4_sheet) # boom! this file is now in my trash folder 
```
...
```{r}
#write_sheet(data = MPS_tracker_data,
           # ss = "15o7DB_kqLc_PbsRkBC9cGjVYd5pGeAJyMrM0TWRUGF8", 
            #sheet = "Sheet1")
```
