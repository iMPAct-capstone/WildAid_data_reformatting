...
final_data <-final_data |> select(year = year.x, category = Final_Category, sub_category = final_name, indicator_type, score, country, site, comments, entered_by) |> mutate(score = as.numeric(score))
...
file_cells <- xlsx_cells(file.path(data_path, "raw", file_list[1]), sheet = 1) #read the cells of each file
format_cells <- xlsx_formats(file.path(data_path, "raw", file_list[1])) #read the format of each file
file_name <- as.character(file_list[1])
...
```{r}
test <- MPS_tracker_data |> group_by(site, year, sub_category) |> 
  summarize(sum(score), n=  n()) |> filter(n>1L)
```
