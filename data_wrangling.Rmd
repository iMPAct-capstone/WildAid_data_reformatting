...
split <- fuzzyjoin::regex_left_join( 
  x = data_reformat, #reformatted data 
  y = sub_category_tab, #lookup table
  by = "sub_category", #joining on sub-category
  ignore_case = TRUE  #make a column for a numeric representation of how related the string are
...
average <- fuzzyjoin::regex_left_join(
  x = data_reformat, 
  y = sub_category_tab, 
  by = "sub_category",
  ignore_case = TRUE,
...
file_cells <- xlsx_cells(file.path(data_path, "raw", file_list[7]), sheet = 1) #read the cells of each file
format_cells <- xlsx_formats(file.path(data_path, "raw", file_list[7])) #read the format of each file
file_name <- as.character(file_list[7])
...
if (year == 2019 & site == "Galapagos") {
problem <- fuzzyjoin::stringdist_join(str_detect(data_reformat, "case_database"), )
}
